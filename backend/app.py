...
#!/usr/bin/env python3
"""
Dallas Appraisal Protest Helper - Flask Backend
Endpoints:
  GET  /api/health
...
DISCLAIMER   = (
    ...
    "It does not constitute legal, tax, or appraisal advice. "
    "Consult a licensed property tax professional before filing a protest."
)
...
def get_comps_data(conn, subject, exclude_new=True, supporting_only=False):
    ...
    bands = [
        (0.75, 1.25, 10),  # Pass 1: +/-25% size, +/-10yr
        (0.65, 1.35, 15),  # Pass 2: +/-35% size, +/-15yr
        (0.50, 1.50, 20),  # Pass 3: +/-50% size, +/-20yr
        (0.0,  9.0,  99),  # Pass 4: full neighborhood, no size/age filter
    ]
    if supporting_only and subj_psf > 0:
        # Progressive widening: find 15 comps below subject $/sqft
        ...
        for pidx, (lo_mult, hi_mult, age_delta) in enumerate(bands, 1):
            ...
            # Score and re-rank by similarity (size 40%, age 30%, psf gap 30%)
        ...
        # Standard mode: same progressive widening as supporting mode, no PSF ceiling
        comps     = []
        used_band = bands[0]
        pass_num  = 1
...
def get_band_desc(comps, sqft_lo, sqft_hi, yr_lo, yr_hi):
    """Return the appropriate band description for the evidence narrative."""
...
def get_band_vals(comps, subject):
    """Extract band metadata from comps (or compute defaults)."""
    if comps:
        return (
...
def property_lookup():
    ...
    try:
        ...
        return jsonify({"count": len(results), "properties": results})
    finally:
        conn.close()
...
def get_comps():
    ...
    exclude_new     = request.args.get("exclude_new", "true").lower() != "false"
    supporting_only = request.args.get("supporting_only", "true").lower() != "false"
    if not account:
    ...
    try:
        ...
        stats["supporting_only"] = supporting_only
        ...
        return jsonify({"subject": subject, "comps": comps, "stats": stats})
    finally:
        conn.close()


@app.route("/api/evidence-html")
def evidence_html():
    account         = request.args.get("account", "").strip()
    owner_name      = request.args.get("owner_name", "Property Owner").strip()
    opinion_str     = request.args.get("opinion_of_value", "").strip()
    exclude_new     = request.args.get("exclude_new", "true").lower() != "false"
    supporting_only = request.args.get("supporting_only", "true").lower() != "false"
    if not account:
        return "account required", 400
    conn = get_db()
    try:
        subject   = get_subject(conn, account)
        if not subject:
            return f"Account {account} not found", 404
        comps     = get_comps_data(conn, subject, exclude_new, supporting_only)
        stats     = compute_stats(subject, comps)
...
def generate_pdf():
    ...
    owner_name      = data.get("owner_name", "Property Owner").strip()
    opinion_str     = str(data.get("opinion_of_value", "")).strip()
    exclude_new     = data.get("exclude_new", True)
    supporting_only = data.get("supporting_only", True)
    if not account:
        return jsonify({"error": "account required"}), 400
    conn = get_db()
    try:
...
if __name__ == "__main__":
    if not DB_PATH.exists():
        ...
        sys.exit(1)
    print(f"Starting Dallas Protest Helper API on http://localhost:5000")
