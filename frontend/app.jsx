...
function CompsScreen({ property, onBack, onNext }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(url);
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Failed loading comps");
      setData(json);
    } catch (e) {
      setError(e.message);
    } finally {
...
function EvidenceScreen({ property, compsData, onBack, onNext, excludeNew: initExclude }) {
  ...
  const [excludeNew, setExcludeNew] = useState(initExclude ?? true);
  ...
  const fetchEvidence = useCallback(async () => {
    setHtmlLoading(true);
    try {
      const url = `${API}/api/evidence-html?account=${property.ACCOUNT_NUM}` +
      ...
      // Extract body content only and strip embedded style block
